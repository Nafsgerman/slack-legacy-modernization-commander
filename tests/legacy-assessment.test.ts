...
test("SME decision buttons flip the demo workflow state visibly in the card", async () => {
  const assessment = await runLegacyAssessmentWorkflow("claims-batch");

  const reviewed = JSON.stringify(
    renderModernizationAssessmentBlocks(assessment, { demoWorkflowStatus: "sme_reviewed" })
  );
  assert.match(reviewed, /SME reviewed for demo session/);
  assert.match(reviewed, /SME review marked complete/);
  assert.match(reviewed, /No persistent enterprise state changed/i);
  ...
  assert.match(followUp, /SME follow-up requested/);
  assert.match(followUp, /Review required before implementation planning/i);

  // Both action paths keep the buttons available for a multi-step demo
  // and never leak raw snake_case status into the rendered card.
  assert.match(reviewed, new RegExp(legacyAssessmentActionIds.markSmeReviewed));
  assert.match(followUp, new RegExp(legacyAssessmentActionIds.needsSmeFollowUp));
  assert.doesNotMatch(reviewed, /sme_required|sme_validated|machine_inferred/);
  assert.doesNotMatch(followUp, /sme_required|sme_validated|machine_inferred/);
});
